...
echo "Setup build host for image-mode operational-updates lab" > /tmp/progress.log
chmod 666 /tmp/progress.log

# Base image the lab builds on. The initial (v1) image the guest boots and the
# updated (v2) image the participant builds in Module 2 both derive from this.
BOOTC_RHEL_VER=10.1
...
# Build the INITIAL (v1) image the guest boots. The participant builds the
# updated (v2) image in Module 2 and pushes to this SAME ref — that shared ref
# is what makes the automatic-update timer and rollback work.
cd ~
CTX=~/image-context
mkdir -p $CTX
cat <<EOF> $CTX/Containerfile
FROM ${BASE}
RUN echo "1.0" > /etc/lab-image-version
...
# from the lab's own git repo via sparse checkout.
cd ~
EXAMPLE=examples
TMPDIR=/tmp/lab
...
echo "Image-mode operational-updates lab setup complete" >> /tmp/progress.log
